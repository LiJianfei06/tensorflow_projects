...
i=0
classes = ["train" ];#目录 分别改成train 和 val 各跑一次就行了
print "start..."
writer = tf.python_io.TFRecordWriter("train.tfrecords")   #分别改成train 和 val 各跑一次就行了
...
for serialized_example in tf.python_io.tf_record_iterator("train.tfrecords"):   #分别改成train 和 val 各跑一次就行了
